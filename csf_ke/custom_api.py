...
def _get_withholding_account(doc, tax_type, posting_side, mode):
	...
	fieldname = f"withholding_{tax_type}_{mode_map.get(mode, mode.lower().replace(' ', '_'))}_{posting_side}_account"
...
def make_purchase_withholding_journal_entries(doc, method=None):
	...
	for item in doc.items:
		...
		if not wtax_payable_account:
			frappe.throw(_("Please set Default Withholding Tax Payable Account in Company {0}").format(doc.company))
	...
	if flt(doc.withholding_vat_amount) and not cint(doc.csf_ke_wvat_je_created):
		if not wvat_payable_account:
			frappe.throw(_("Please set Default Withholding VAT Payable Account in Company {0}").format(doc.company))
...
def make_sales_withholding_journal_entries(doc, method=None):
	...
	for item in doc.items:
		...
		if not wtax_receivable_account:
			frappe.throw(_("Please set Default Withholding Tax Receivable Account in Company {0}").format(doc.company))
	...
	if flt(doc.withholding_vat_amount) and not cint(doc.csf_ke_wvat_je_created):
		if not wvat_receivable_account:
			frappe.throw(_("Please set Default Withholding VAT Receivable Account in Company {0}").format(doc.company))
